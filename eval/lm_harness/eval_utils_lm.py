...
class BaseLM(LM):
    ...
    def loglikelihood(self, requests):
        new_reqs = []
        pbar = tqdm(total=len(requests), desc="MMLU loglikelihood", dynamic_ncols=True, miniters=1)
        
        for inst in requests:
            # assert isinstance(inst, Instance)
            context, continuation = inst.args     # (context, continuation)
    ...
    def loglikelihood_rolling(self, requests):
        ...
        for req in tqdm(requests):
            if isinstance(req, tuple):
                ...
                args = req.args
                if not args:
                    raise ValueError("Instance args is empty for loglikelihood_rolling.")
                string = args[0]
            else:
                raise TypeError(
                    "Requests must be tuple(string,) or Instance with args[0] string."
                )
...
class LMEvalAdaptor(BaseLM):
    ...
    def generate_until(self, requests):
        # Use BaseLM's default implementation, it will call your implemented _model_generate
        # return BaseLM.generate_until(self, requests)
...
def generate_until(self, requests):
    outputs = []
    # for req in requests:
    for req in tqdm(requests, desc="LMEval Generating", total=len(requests), ncols=100):
        prompt, gen_kwargs = req.args  # Tuple[str, dict]

        # Generation parameters
        until = gen_kwargs.get("until", []) or []
        max_gen_toks = min(
            int(gen_kwargs.get("max_gen_toks", self.max_gen_toks)),
            self.max_gen_toks,
        )
        ...
        gen_ids = self.model.generate(
            ...
            do_sample=do_sample,
            use_cache=True,
            attention_mask=enc["attention_mask"],  # Critical
            pad_token_id=self.tokenizer.pad_token_id,
        )[0]

        full = self.tokenizer.decode(gen_ids, skip_special_tokens=False)
